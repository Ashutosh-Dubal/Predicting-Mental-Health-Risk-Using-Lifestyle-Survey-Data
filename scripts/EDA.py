...
for target, title in targets.items():
    ...
    top10_features = corr[target].drop(target).abs().sort_values(ascending=False).head(10).index
    ...
    top_corr = corr[target].drop(target).sort_values(key=abs, ascending=False).head(10)
